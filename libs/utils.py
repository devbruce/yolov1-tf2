...
def postprocess_yolo_format(yolo_pred_boxes, input_height, input_width, cell_size, boxes_per_cell):
    """Postprocess YOLO Format Boxes
    Args:
      yolo_pred_boxes (EagerTensor dtype=tf.float32): 4-D tensor [cell_size, cell_size, boxes_per_cell, 5] ==> [cx_in_cell, cy_in_cell, w_sqrt, h_sqrt, confidence]
    ...
    for y_grid_idx in range(cell_size):
        for x_grid_idx in range(cell_size):
            ...
            for i in range(boxes_per_cell):
                box = cell[i]
                cx_in_cell, cy_in_cell, w_sqrt, h_sqrt, confidence = box[0], box[1], box[2], box[3], box[4]
